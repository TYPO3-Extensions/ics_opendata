 ...
 */
class tx_icsopendatastore_module1 extends t3lib_SCbase {
	var $pageinfo;
...
// - columnsOnly[table]: Liste des champs à afficher pour table. Les champs de overrideVals sont masqués.
	...
	function moduleContent()	{
		...
		$this->cmd_table = 'tx_icsopendatastore_files';
		...
		$dblist->start($this->pid,'tx_icsopendatastore_files',$this->pointer,'',0,$this->showLimit);
	...
	function renderMenuButtons()	{
		...
		$sContent .= '<p style="display: inline; margin-left: 1em"><a href="#" onclick="' . htmlspecialchars(t3lib_BEfunc::editOnClick($GLOBALS['SOBE']->getNewRecordParams(),$this->backPath,-1)) . '">' .
			'<img ' .  t3lib_iconWorks::skinImg(t3lib_extMgm::extRelPath('ics_opendata_store'), 'res/uploader.png') . ' title="' . $LANG->getLL('newFile') . '" alt="' . $LANG->getLL('newFile') . '" />' .
			'</a></p>';
		$sContent .= '<p style="display: inline; margin-left: 1em"><a href="#" onclick="' . htmlspecialchars(t3lib_BEfunc::editOnClick('&edit[tx_icsopendatastore_filegroups][' . $this->filegroup['uid']. ']=edit',$this->backPath,-1)) . '">' .
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata_store/mod1/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata_store/mod1/index.php']);
}
...
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_icsopendatastore_module1');
$SOBE->init();
