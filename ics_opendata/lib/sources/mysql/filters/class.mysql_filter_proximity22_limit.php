...
/**
 * filter proximity22 limit : add a where and a limit to the mysql query
 *
 * @author	Mathias Cocheri <mcocheri@in-cite.net>
 * @package	TYPO3
 * @subpackage	tx_icsopendata
 */
class tx_icsopendata_mysql_filter_proximity22_limit
	implements tx_icsopendata_filter_generator
{
	public function generateFilter($filter, $nbindent)
	{
		...
		$returnarray['LIMIT'] = $indent . '$params[\'' . $filter->getParam(2) . '\']';
		
		return $returnarray;
	}

} /* end of class tx_icsopendata_mysql_filter_proximity22_limit */

if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata/lib/sources/mysql/filters/class.mysql_filter_proximity22_limit.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata/lib/sources/mysql/filters/class.mysql_filter_proximity22_limit.php']);
}
