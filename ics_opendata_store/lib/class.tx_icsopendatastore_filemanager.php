...
class tx_icsopendatastore_filemanager {
	...
	function processDatamap_preProcessFieldArray(&$pi_aIncommingFieldArray, $pi_sTable, $pi_sId, $pi_oTce) {
		...
		if ($pi_sTable == 'tx_icsopendatastore_files') {
			$_EXTCONF = unserialize($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['ics_opendata_store']);
			...
			if (isset($pi_aIncommingFieldArray['filegroup'])) {
				...
				$aRes = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
					'uid_foreign',
					'tx_icsopendatastore_files_filegroups_mm',
					'uid_local = ' . $pi_sId
				);
				$sFilegroup = $aRes['0'];
			}
	...
	// function processDatamap_postProcessFieldArray($pi_sStatus, $pi_sTable, $pi_sId, &$pi_aFieldArray, $pi_oTce) {
		// if ($pi_sTable == 'tx_icsopendatastore_files') {
			// if ((count($pi_aFieldArray) > ((isset($pi_aFieldArray[$GLOBALS['TCA']['tx_icsopendatastore_files']['ctrl']['enablecolumns']['disabled']])) ? (1) : (0))) && is_numeric($pi_sId)) {
