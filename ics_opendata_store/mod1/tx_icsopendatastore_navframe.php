 ...
 *
 *
 *   64: class  tx_icsopendatastore_navframe
 *   76:     function init()
...
$LANG->includeLLFile('EXT:ics_opendata_store/mod1/locallang.xml');

/**
 * Module 'Documents' for the 'ics_opendata_store' extension.
 *
 * @author	DSIT Ville de Rennes <technique@in-cite.net>
 * @package	TYPO3
 * @subpackage	tx_icsopendatastore
 */
class  tx_icsopendatastore_navframe {
	...
	function init()	{
		...
		$this->include_once[] = t3lib_extMgm::extPath('ics_opendata_store') . 'lib/class.tx_icsopendatastore_sysfolder.php';

		$this->doc = t3lib_div::makeInstance('template');
		$this->doc->backPath = $BACK_PATH;
		$this->doc->setModuleTemplate(t3lib_extMgm::extRelPath('ics_opendata_store') . 'mod1/mod_navframe.html');
	...
	function main()	{
		...
		if (!$GLOBALS['BE_USER']->check('tables_select', 'tx_icsopendatastore_filegroups'))
		{
		...
		$pid = tx_icsopendatastore_sysfolder::getPid();
		if (is_null($pid))
		{
			$oMessage = t3lib_div::makeInstance(
				't3lib_FlashMessage',
		...
		$sOnclickNewFilegroup = 'top.content.list_frame.location.href=top.TS.PATH_typo3+\'alt_doc.php?edit[tx_icsopendatastore_filegroups][' . $pid . ']=new&returnUrl=\' + encodeURIComponent(top.content.list_frame.location.href);';
		$this->markers['NEW_PAGE'] = '<a href="#" onclick="' . htmlspecialchars($sOnclickNewFilegroup) . '"><img' . t3lib_iconWorks::skinImg($GLOBALS['BACK_PATH'], 'gfx/new_el.gif') . ' title="' . $LANG->sL('LLL:EXT:ics_opendata_store/mod1/locallang.xml:new_filegroup', 1) . '" alt="" /></a>';
	...
	{
		global $TYPO3_DB;
		$sTable = 'tx_icsopendatastore_filegroups';
	...
	function printContent()	{
		...
		$this->content = $this->doc->startPage($LANG->sL('LLL:EXT:ics_opendata_store/mod1/locallang_mod.xml:mlang_labels_tablabel',1));
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata_store/mod1/tx_icsopendatastore_navframe.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata_store/mod1/tx_icsopendatastore_navframe.php']);
}
...
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_icsopendatastore_navframe');
$SOBE->init();
