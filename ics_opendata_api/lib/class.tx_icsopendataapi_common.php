...
/**
 * Plugin 'Common for plugins' for the 'ics_opendata_api' extension.
 *
 * @author	Tsi <tsi@in-cite.net>
 * @package	TYPO3
 * @subpackage	tx_icsopendataapi
 */
class tx_icsopendataapi_common extends tslib_pibase {
	
	const APPMODE_ALL = 0;
	const APPMODE_USER = 1;
	const APPMODE_SINGLE = 2;
	const APPMODE_SINGLEUSER = 3;
	const APPMODE_SEARCH = 4;
	const APPMODE_MAX = 4;
	
	var $templateFile = "typo3conf/ext/ics_opendata_api/res/template.html";
	var $tables = array(
		'applications' => 'tx_icsopendataapi_applications',
		'statistics' => 'tx_icsopendataapi_statistics',
		'users' => 'fe_users',
	);
	 ...
	 * __Constructor
	 */
	function tx_icsopendataapi_common() {
	...
	function getApplications($mode, $selectFields = null, $parameter = null) {
		...
		switch($mode) {
			case self::APPMODE_ALL:
				...
				$addWhere[] = '`'.$this->tables['applications'].'`.`publication_date` > 0 ';
				$addWhere[] = '`'.$this->tables['applications'].'`.`publication_date` < '.time();
			...
			case self::APPMODE_SINGLE:
				...
				if (!$GLOBALS['TSFE']->fe_user->user['uid']) {
					...
					$addWhere[] = ' (
						...
						OR (
							`'.$this->tables['applications'].'`.`publication_date` > 0 AND 
							`'.$this->tables['applications'].'`.`publication_date` < '.time().' AND 
		...
		if (!$selectFields) {
			$select = '`'.$this->tables['applications'].'`.`uid`, 
				...
				`'.$this->tables['applications'].'`.`key_appli`, 
				`'.$this->tables['applications'].'`.`application`, 
				`'.$this->tables['applications'].'`.`description`, 
				`'.$this->tables['applications'].'`.`platform`, 
				`'.$this->tables['applications'].'`.`count_use`, 
				`'.$this->tables['applications'].'`.`max`, 
				`'.$this->tables['applications'].'`.`publication_date`, 
	...
	function getStatistics($selectFields = '', $whereFields = array(), $addWhereText = '', $groupby = '', $order = '', $limit = '', $debugger = false) {
		...
		if (!$selectFields) {
			$selectFields = '`'.$this->tables['statistics'].'`.`uid`, `'.$this->tables['statistics'].'`.`tx_icsopendataapi_application`, `'.$this->tables['statistics'].'`.`cmd`, `'.$this->tables['statistics'].'`.`count`, `'.$this->tables['statistics'].'`.`date`';
		}
