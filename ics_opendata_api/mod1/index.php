...
require_once( t3lib_extMgm::extPath('ics_opendata_api') . 'lib/class.tx_icsopendataapi_command.php' );
...
require_once( t3lib_extMgm::extPath('ics_opendata_api') . 'mod1/class.tx_icsopendataapi_module1_menu.php' );
require_once( t3lib_extMgm::extPath('ics_opendata_api') . 'mod1/class.tx_icsopendataapi_module1_command.php' );


/**
 * Module 'Commands documentation' for the 'ics_opendata_api' extension.
 *
 * @author	Tsi <tsi@in-cite.net>
 * @package	TYPO3
 * @subpackage	tx_icsopendataapi
 */
class  tx_icsopendataapi_module1 extends t3lib_SCbase {
	var $pageinfo;
	...
	function main()	{
		...
		if (($this->id && $access) || ($BE_USER->user['admin'] && !$this->id))	{
				// Draw the header.
			$this->doc->form='<form name="tx_icsopendataapi_module1" id="tx_icsopendataapi_module1" action="" method="post" enctype="multipart/form-data">';
			...
			. '</script>';
			$this->doc->inDocStyles = file_get_contents( t3lib_extMgm::extPath('ics_opendata_api') . '/mod1/styles.css');
	...
	function moduleContent(){
		...
		$extConf = unserialize($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['ics_opendata_api']);
		if ($extConf && $extConf['xmldoc.'])	{
			...
			if( isset($POST['hidden'])
				...
				$menu = new tx_icsopendataapi_module1_menu();
				$menu->load($command);
				...
				$cmd = new tx_icsopendataapi_module1_command();
	...
	function deleteElement( &$element, $delete){
		if( $delete[0] == 'command' ){
			...
			if( (count($delete) == 1) ){
				...
				if( get_class($element) == 'tx_icsopendataapi_parameter' ){
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata_api/mod1/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata_api/mod1/index.php']);
}
...
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_icsopendataapi_module1');
$SOBE->init();
