 ...
 */
class tx_icsopendataapi_pi4 extends tx_icsopendataapi_common {
	...
	function detailView($content,$row){
		...
		$markerArray = array(
			...
			'###DESCRIPTION###' => $this->renderText($row['description']),
		...
		if(!$html || $html == '')
			...
			foreach($rows as $k => $row){
				...
				$markerArray = array(
					...
					'###DOWNLOAD###' => $row['count_use'],
					'###PUBLISH_DATE###' => strftime("%d/%m/%Y",$row['publication_date']),
					'###DESCRIPTION###' => $this->renderText($row['description'],$this->conf['list.']['descSize']),
	 ...
	 * @return text
	 */
	function renderText ($text, $limit = 0) {
		if($limit >0 ) {
			$text = t3lib_div::fixed_lgd(strip_tags($text),$limit);
		} else {
			$text = strip_tags($text);
			$text = ereg_replace("(http[s]?://)([^<>,[:space:]]+[[:alnum:]/])", "<a href=\"\\1\\2\" target=\"_blank\">\\1\\2</a>", $text);
		}
		$text = nl2br($text);
		return $text;
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata_api/pi4/class.tx_icsopendataapi_pi4.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ics_opendata_api/pi4/class.tx_icsopendataapi_pi4.php']);
}
