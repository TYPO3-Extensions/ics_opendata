...
/**
 * @file class.tx_userdatastore_datastore_getdatasets_command.php
 *
...
class tx_userdatastore_datastore_getdatasets_command extends tx_icsopendataapi_command
{
	...
	function execute(array $params, XMLWriter $xmlwriter)
	{
		...
		if (empty($params['ids']))
		{
			makeError($xmlwriter, tx_userdatastore_datastore_getdatasets_command::EMPTY_IDS_CODE, tx_userdatastore_datastore_getdatasets_command::EMPTY_IDS_TEXT);
			return;
		}
		...
		foreach ($ids as $id)
		{
			if (!is_numeric($id))
			{
				makeError($xmlwriter, tx_userdatastore_datastore_getdatasets_command::INVALID_IDS_CODE, tx_userdatastore_datastore_getdatasets_command::INVALID_IDS_TEXT);
				return;
			}
		}
		if (empty($params['type']))
		{
			makeError($xmlwriter, tx_userdatastore_datastore_getdatasets_command::EMPTY_TYPE_CODE, tx_userdatastore_datastore_getdatasets_command::EMPTY_TYPE_TEXT);
			return;
		}
		if (!in_array($params['type'], tx_userdatastore_datastore_getdatasets_command::$types))
		{
			makeError($xmlwriter, tx_userdatastore_datastore_getdatasets_command::INVALID_TYPE_CODE, tx_userdatastore_datastore_getdatasets_command::INVALID_TYPE_TEXT);
			return;
		}
		if (empty($params['filetype']))
		{
			makeError($xmlwriter, tx_userdatastore_datastore_getdatasets_command::EMPTY_FILETYPE__CODE, tx_userdatastore_datastore_getdatasets_command::EMPTY_FILETYPE__TEXT);
			return;
		}
		if (!in_array($params['filetype'], tx_userdatastore_datastore_getdatasets_command::$filetypes))
		{
			makeError($xmlwriter, tx_userdatastore_datastore_getdatasets_command::INVALID_FILETYPE_CODE, tx_userdatastore_datastore_getdatasets_command::INVALID_FILETYPE_TEXT);
			return;
		}
		...
		$datasource = t3lib_div::getUserObj($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['user_datastore']['datasource']['dataset']);
	...
	protected function transformResultsForOutput(array $datasets)
	{
		...
		foreach ($datasets as $dataset)
		{
			...
			// $element['categories'] = $dataset['categories'];
...
} // End of class tx_userdatastore_datastore_getdatasets_command
